...
class GPT {
  ...
  static GPT? loadFromFile(String path, Random rng) {
    final file = File(path);
    if (!file.existsSync()) {
      return null;
    }
    ...
    final model = GPT(
      ...
      nEmbed: data['nEmbed'],
      nHead: data['nEmbed'] ~/ 32, // Assume 32 dim per head
      nLayer: data['nLayer'],
      rng: rng,
    );
    ...
    params.forEach((name, tensor) {
      if (data.containsKey(name)) {
        final tensorData = data[name] as Map<String, dynamic>;
        final loadedData = (tensorData['data'] as List).cast<double>();
        for (int i = 0; i < tensor.data.length && i < loadedData.length; i++) {
          tensor.data[i] = loadedData[i];
        }
      }
    });
...
void main(List<String> args) async {
  ...
  final blockSize = 64;
  final modelPath = 'model.json';
  final checkpointPath = 'checkpoint.json';
  final logPath = 'training.log';
  
  // Initialize log file
  final logFile = File(logPath);
  logFile.writeAsStringSync('Training Log Started\n', mode: FileMode.write);
  
  // Try to load existing model
  var model = GPT.loadFromFile(modelPath, rng);
  var startIter = 1;
  ...
  if (model == null) {
    ...
    if (checkpointFile.existsSync()) {
      final checkpointData = jsonDecode(checkpointFile.readAsStringSync());
      startIter = checkpointData['iteration'] + 1;
      print('Resuming training from iteration $startIter');
    }
  }
  ...
  print('Starting training from iteration $startIter to $iters');
  for (int it = startIter; it <= iters; it++) {
    final batch = makeBatches(train, blockSize, batchSize, rng);
    final xs = batch[0], ys = batch[1];
    
    // Log training data every 10 iterations
    if (it % 10 == 0 || it == 1) {
      logFile.writeAsStringSync('\n--- Iteration $it Training Data ---\n', mode: FileMode.append);
      for (int b = 0; b < min(2, xs.length); b++) { // Log first 2 batches
        final inputSeq = xs[b].take(min(20, xs[b].length)).toList(); // First 20 tokens
        final targetSeq = ys[b].take(min(20, ys[b].length)).toList();
        final inputText = tok.decode(inputSeq);
        final targetText = tok.decode(targetSeq);
        logFile.writeAsStringSync('Batch $b Input: "$inputText" (tokens: $inputSeq)\n', mode: FileMode.append);
        logFile.writeAsStringSync('Batch $b Target: "$targetText" (tokens: $targetSeq)\n', mode: FileMode.append);
      }
    }
    ...
    // Log predictions vs targets every 10 iterations
    if (it % 10 == 0 || it == 1) {
      final B = logits.shape[0], T = logits.shape[1], V = logits.shape[2];
      final flat = Tensor.reshape(logits, [B * T, V]);
      final probs = Tensor.softmaxLastDim(flat);
      
      logFile.writeAsStringSync('--- Model Predictions vs Targets ---\n', mode: FileMode.append);
      for (int b = 0; b < min(1, B); b++) { // Log first batch only
        for (int t = 0; t < min(10, T); t++) { // First 10 positions
          final idx = b * T + t;
          final targetToken = ys[b][t];
          
          // Find top predicted token
          double maxProb = -1;
          int predToken = 0;
          for (int v = 0; v < V; v++) {
            if (probs.data[idx * V + v] > maxProb) {
              maxProb = probs.data[idx * V + v];
              predToken = v;
            }
          }
          
          final targetChar = tok.decode([targetToken]);
          final predChar = tok.decode([predToken]);
          final targetProb = probs.data[idx * V + targetToken];
          
          logFile.writeAsStringSync('Pos $t: Target="$targetChar"($targetToken, p=${targetProb.toStringAsFixed(4)}) Pred="$predChar"($predToken, p=${maxProb.toStringAsFixed(4)}) ${targetToken == predToken ? "✓" : "✗"}\n', mode: FileMode.append);
        }
      }
    }
    ...
    if (it % 10 == 0 || it == 1) {
      ...
      vLoss = batchLoss(cel, vLogits, vbatch[1]);
      print('iter $it/$iters train=${loss.data[0].toStringAsFixed(4)} val=${vLoss.data[0].toStringAsFixed(4)} (${((it / iters) * 100).toStringAsFixed(1)}%)');
      stdout.flush();
    } else {
      print('iter $it/$iters train=${loss.data[0].toStringAsFixed(4)} (${((it / iters) * 100).toStringAsFixed(1)}%)');
      stdout.flush();
    }
    
    // Log to file
    final logEntry = 'iter $it/$iters train=${loss.data[0].toStringAsFixed(4)}${vLoss != null ? ' val=${vLoss.data[0].toStringAsFixed(4)}' : ''} (${((it / iters) * 100).toStringAsFixed(1)}%)\n';
    logFile.writeAsStringSync(logEntry, mode: FileMode.append);
    
    // Generate sample every 25 iterations
    if (it % 25 == 0) {
      final samplePrompt = 'Alice ';
      final sampleIds = tok.encode(samplePrompt).toList();
      final sampleGen = sample(model, sampleIds, 50, rng);
      final sampleOut = tok.decode(sampleGen);
      
      print('Sample at iter $it: "$sampleOut"');
      logFile.writeAsStringSync('\n--- Sample Generation at Iteration $it ---\n', mode: FileMode.append);
      logFile.writeAsStringSync('Prompt: "$samplePrompt"\n', mode: FileMode.append);
      logFile.writeAsStringSync('Generated: "$sampleOut"\n', mode: FileMode.append);
    }
    
    // Save checkpoint every 5 iterations
    if (it % 5 == 0) {
      model.saveToFile(modelPath);
      final checkpoint = {'iteration': it};
      File(checkpointPath).writeAsStringSync(jsonEncode(checkpoint));
      print('Checkpoint saved at iteration $it');
      logFile.writeAsStringSync('Checkpoint saved at iteration $it\n', mode: FileMode.append);
    }
  }

  // Final save
  model.saveToFile(modelPath);
  File(checkpointPath).writeAsStringSync(jsonEncode({'iteration': iters}));
  print('Training completed! Final model saved.');
  logFile.writeAsStringSync('Training completed! Final model saved.\n', mode: FileMode.append);
  ...
  print('Prompt: "$prompt"');
  logFile.writeAsStringSync('\n=== Generating Sample ===\n', mode: FileMode.append);
  logFile.writeAsStringSync('Prompt: "$prompt"\n', mode: FileMode.append);
  ...
  print('Generated: "$out"');
  stdout.writeln('\n=== Sample ===\n$out');
  logFile.writeAsStringSync('Generated: "$out"\n', mode: FileMode.append);
  logFile.writeAsStringSync('\n=== Sample ===\n$out\n', mode: FileMode.append);
}

String _loadToyText() {
  try {
    final file = File('clean_corpus.txt');
    if (file.existsSync()) {
      final content = file.readAsStringSync();
      print('Loaded ${content.length} characters from combined corpus');
      return content;
    }
  } catch (e) {
    print('Failed to load corpus file: $e');
  }
  
  // Fallback to original text if file loading fails
  final s = '''
